...
class BeamFormer(object):
    ...
    def create_beam(self, mic_signals:np.ndarray[np.ndarray], angle_deg:float)->np.ndarray[float]:
        ...
        for channel_index in range(len(mic_signals)):
            ...
            if filter.buffer_index >= 2:
                ...
                # perform filtering on the whole 3x buffer
                # filtered_signal, z = signal.lfilter(filter._numerators, filter._denominators, filter.working_buffer, zi=filter._filter_state)
                filtered_signal = signal.lfilter(filter._numerators, filter._denominators, filter.working_buffer)
                # filter._filter_state = z
                                
                
                # extract the "middle" buffer by removing the extra samples from the con
                shifted_signal = filtered_signal[constants.buffer_length:-constants.buffer_length]
    ...
    def get_beam_filter(self, mic_index:int, beam_angle_deg:float) -> MicrophoneBeamFilter:
        ...
        N = 2   # order of the IIR all pass filter
        mic = self._microphone_positions[mic_index]
        # print(mic)
        ...
        _numerators = np.flip(_denominators[1:])
