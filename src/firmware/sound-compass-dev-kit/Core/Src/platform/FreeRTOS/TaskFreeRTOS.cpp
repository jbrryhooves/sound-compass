...
platform::ITaskFactory::TaskHandle platform::FreeRTOS::TaskFreeRTOSFactory::createTaskStatic(const char *taskName, void *taskControlBlock, unsigned int controlBlockSize,
        void *stackMemory, unsigned int stackSize, int priority, ITask *task)
{
    if (stackSize < configMINIMAL_STACK_SIZE)
    {
        stackSize = configMINIMAL_STACK_SIZE;
    ...
    { .name = taskName, .cb_mem = taskControlBlock, .cb_size = controlBlockSize, .stack_mem = stackMemory, .stack_size = stackSize, .priority = (osPriority_t) priority };
