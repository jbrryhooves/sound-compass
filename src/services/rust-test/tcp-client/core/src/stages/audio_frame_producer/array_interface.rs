...
use crate::stages::{audio_frame_producer::message_parser, messages::AudioFrame};
...
fn receive_loop(
    stop_channel: Receiver<bool>, audio_frame_channel: Sender<AudioFrame>,
) -> Result<()> {
    let (mut _tx_from_tcp, mut rx_from_tcp): (Sender<Vec<u8>>, Receiver<Vec<u8>>) =
    ...
    let mut parser = message_parser::MessageParser::new();
    ...
    loop {
        ...
        // println!("bytes: {:x?} ", &val[..val.len()]);

        match parser.parse_bytes(&val) {
            Some(message) => {
                match message {
                    crate::stages::MessageTypes::AudioFrame(frame) => {
                        audio_frame_channel.send(frame);
                    }
                    crate::stages::MessageTypes::RuntimeMetrics(_) => {}
                }
            }
            None => {}
        }
